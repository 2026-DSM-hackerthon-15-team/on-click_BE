...
- AI 실연동: `AI_PROVIDER=http`, `AI_BASE_URL`, timeout 및 기능별 path (`AI_CHAT_PATH`, `AI_DAILY_CONSULTING_PATH`, `AI_INSTAGRAM_PUBLISH_PATH` 등)
- 미디어: AI 서버가 조회할 수 있는 HTTPS `MEDIA_PUBLIC_BASE_URL`
...
마감 매출·내일 방문자 예측에 사용할 POS 판매 데이터가 없으면 `404 POS_DATA_NOT_FOUND`를 반환합니다. 오늘 요약과 시간대별 통계는 데이터가 없어도 합계 0과 빈 시간대 집계를 `200`으로 반환합니다.
...
4. 콘텐츠를 승인하면 메인 백엔드가 승인한 사용자의 JWT와 게시 정보를 AI 서버에 전달합니다.
5. AI 서버가 브라우저 MCP로 Instagram 웹 게시를 완료하고, 승인 응답은 최종 `PUBLISHED` 또는 `FAILED` 상태를 반환합니다.
...
`AI_PROVIDER=http`에서는 승인 API가 `POST /ai/marketings/{marketingId}/publish/instagram`을 동기 호출합니다. 이 호출에만 승인 요청의 `Authorization: Bearer` 토큰을 그대로 전달하며 다른 AI API 인증 방식은 변경하지 않습니다. 부작용이 있는 게시 요청은 HTTP 계층에서 자동 재시도하지 않습니다. AI 응답은 최종 `PUBLISHED` 또는 `FAILED`여야 하며 완료 통로가 없는 `PROCESSING` 응답은 잘못된 응답으로 처리합니다.

마케팅 문구 생성과 Instagram 게시 모두 `MEDIA_PUBLIC_BASE_URL`을 AI 서버와 브라우저 MCP가 인터넷에서 조회할 수 있는 HTTPS 주소로 설정해야 합니다. 이미지는 Docker `media-data` 볼륨에 계속 저장하고 공개 URL을 전달하므로 별도 MinIO는 필요하지 않습니다. 타임아웃처럼 게시 성공 여부를 확정할 수 없는 실패는 로컬 상태를 `FAILED`로 기록하며, 재승인 전에 실제 Instagram 게시 여부를 확인해야 합니다.
...
- 마케팅은 업로드 → 초안 생성/수정 → 승인 순서로 연동합니다. 승인은 Instagram 게시가 끝날 때까지 동기 처리되며 최종 상태를 반환합니다.
